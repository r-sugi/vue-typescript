<template>
  <div class="cp_ipselect">
    <select class="cp_sl06" @change="onChange($event)" required>
    ...
    <label class="cp_sl06_selectlabel">{{ label }}</label>
  </div>
</template>
<script lang="ts">
import Vue from "vue";
export default Vue.extend({
  ...
  methods: {
    onChange($e) {
      const option = this.options.find((o: any) => o.value == $e.target.value);
